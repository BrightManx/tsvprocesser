...
def process_n_nuc(tsv, cof = 'Nuclei - Number of Objects'):    
    ...
    header = df.loc[::n1, ['cell_name', 'day']].sort_values(by=['cell_name', 'day'])

    # Transpose accordingly
    a = pd.DataFrame() 
    for cell_name in df.cell_name.unique():
        for day in df.day.unique():
            b = df.loc[(df.day == day) & (df.cell_name == cell_name), [cof]].transpose()
    ...
    demo = pd.concat([header, a], axis = 1)
